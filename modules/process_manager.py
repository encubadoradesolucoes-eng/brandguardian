...
def register_bpi_event(data: dict, user_id: int = None):
    """
    Registra um novo evento do BPI e atualiza o processo central.
    ETAPA 10: Auditoria (user_id)
    """
    process_number = data.get('process_number')
    if not process_number:
        return None, "Número de processo ausente."

    # Normalização dos dados (ETAPA 4 & 5)
    from modules.normalization import NormalizationMotor
    name = NormalizationMotor.normalize_company_name(data.get('name', ''))
    status = NormalizationMotor.normalize_status(data.get('status', ''))
    filing_date = NormalizationMotor.normalize_date(data.get('filing_date', ''))
    ...
    if not brand:
        ...
        brand = Brand(
            ...
            owner_name=NormalizationMotor.normalize_company_name(data.get('owner_name', '')),
    ...
    # 2. Criar o Evento (BrandLog) - ETAPA 10 AUDITORIA
    log = BrandLog(
        brand_id=brand.id,
        user_id=user_id,
